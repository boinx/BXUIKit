...
function download_for_platform
{
  ...
  curl -OL --fail --user "${GH_TOKEN}:" "https://github.com/boinx/BXSwiftUtils/releases/download/${_tag}/BXSwiftUtils-${_platform}.framework.zip"
  unzip -o *.zip
  popd
}

# Find out the latest tag that was released on Github, because the tag needs to be inserted into the download URls.
_tag=$(curl -s --fail --user "${GH_TOKEN}:" https://api.github.com/repos/boinx/BXSwiftUtils/releases/latest | jq -r '.tag_name')
